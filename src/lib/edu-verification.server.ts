 ...
 * tester accounts) sign in without typing a password, but they must
 * prove they own the address by opening the one-time code we email to
 * it. Student (.edu) accounts always type their own password.
 * ------------------------------------------------------------------ */
export async function internalCodeRequest(email: string) {
  if (!isInternalEmail(email))
    return { ok: false as const, message: "That address needs a password." };
  return requestEmailCode(email);
}

export async function internalCodeVerify(email: string, code: string) {
  if (!isInternalEmail(email))
    return { ok: false as const, message: "That address needs a password." };
  const clean = String(code ?? "").replace(/\D/g, "").slice(0, 10);
  if (!clean) return { ok: false as const, message: "Enter the code we emailed you." };
